Translations for collective.portlet.actions
===========================================
...
templates, you may need to run::
  ...
  (i18ndude)$ cd .../collective/portlet/actions/locales
  (i18ndude)$ python updatelocales.py

If some msgids are missing from the synched ".po" files, you may need to add
these msgids to "collective.portlet.actions-manual.pot" (for
'collective.portlet.actions' domain). Then re-run the above commands.

Having collective.portlet.actions in your language
--------------------------------------------------
...
Easy::
  ...
  $ cd .../collective/portlet/actions/locales # Here ;)
  $ mkdir -p xx/LC_MESSAGES
  $ cp collective.portlet.actions.pot xx/LC_MESSAGES/collective.portlet.actions.po
  $ $EDITOR xx/LC_MESSAGES/collective.portlet.actions.po
...
got collective.portlet.actions with a subversion checkout.
